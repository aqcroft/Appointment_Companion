...
function activeData(selector,key,fallback){
  var el=document.querySelector(selector+'.on')||document.querySelector(selector+'[aria-pressed="true"]');
...
function openShared(raw){
  try{
    ...
    applyPayload(p);
    $('resetSharedBtn').onclick=function(){applyPayload(startPayload);showToast('Starting figures restored')};
    setTimeout(function(){
      $('personalSplash').classList.add('fade');
      setTimeout(function(){$('personalSplash').style.display='none'},380);
    },1500);
  }catch(e){
...
if(/^p=/.test(hash))openShared(hash.slice(2));
