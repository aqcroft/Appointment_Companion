...
(function (global) {
  ...
  var EV_TOP = new Set(('schema_version view_type partner_id customer_name basket_url electricityUsageTotalKwh electricityProfile electricityUsageDayKwh electricityUsageNightKwh ev_state').split(' '));
  ...
  function cleanPartnerId(value) { return String(value || '').trim().toLowerCase().replace(/[^a-z0-9._-]+/g, '').slice(0, 60); }
  ...
  function ev(source) {
    ...
    out.partner_id = cleanPartnerId(input.partner_id); if (!out.partner_id) delete out.partner_id;
    out.customer_name = String(input.customer_name || '').trim().replace(/\s+/g, ' ').slice(0, 80);
    if (input.basket_url !== undefined) { out.basket_url = safeHttps(input.basket_url); if (!out.basket_url) delete out.basket_url; }
