...
(function () {
  ...
  function setBackupIcon(id, good, title) {
    ...
    if (id === 'cloudConnectionState') {
      el.innerHTML = '<img src="../cloud/status-icons/cloud-' + state + '.svg" alt="" aria-hidden="true">';
    } else {
      el.innerHTML = '<picture><source media="(any-pointer: coarse), (max-width: 620px)" srcset="../cloud/status-icons/mobile-' + state + '.svg"><img src="../cloud/status-icons/desktop-' + state + '.svg" alt="" aria-hidden="true"></picture>';
    }
    ...
    card.innerHTML = `
      ...
      <div class="cloudbar">
        <div class="cloudicons">
          <button class="cloudicon cloud-top-shortcut" type="button" id="cloudCustomerShortcut" data-cloud-action="recent-customers" title="Recent customers" aria-label="Recent customers">👥</button>
      ...
      <div id="cloudPilotStatus" class="sr-status" aria-live="polite">Not connected.</div>
  ...
  function openRecentCustomers() { ensureRecentCustomersModal(); renderRecentCustomers(); $c('cloudRecentModal').classList.add('open'); }
  function closeRecentCustomers() { const modal = $c('cloudRecentModal'); if (modal) modal.classList.remove('open'); }
  ...
  function openConnectModal() {
    ensureConnectModal();
    if (window.__AC_LOCAL_ONLY) {
      setStatus('Cloud is paused in local-only mode. Use “Return to normal mode” before reconnecting.', '');
      return;
    }
    $c('cloudPilotPartnerId').value = localStorage.getItem(PARTNER_ID_KEY) || '';
  ...
  function ensureSettingsModal() {
    ...
    modal.innerHTML = `
      <div class="basket-prompt-card" style="max-width:440px;">
        ...
        <div class="cloud-settings-pane" data-settings-pane="local">
          <button class="btn-ghost cloud-settings-back" type="button" data-settings-section="hub">← Settings</button>
          <p class="sub">This device is the working source during an appointment. Cloud synchronises in the background when available.</p>
          <div class="cloud-settings-list">
    ...
    $c('cloudSettingsReload').addEventListener('click', () => {
      closeSettingsModal();
      if (window.__AC_LOCAL_FIRST && window.AppointmentCompanionLocalFirst) {
        window.AppointmentCompanionLocalFirst.scheduleSync(0);
        setStatus('Checking this device against Cloud in the background…');
      } else reloadCurrent();
    });
  ...
  async function boot() {
    buildPanel();
    if (window.__AC_LOCAL_FIRST) {
      const auth = getAuth();
      currentCloudSynced = false;
      setBackupIcon('cloudLocalState', false, 'Checking the local customer database');
      showConnected(!!auth);
      setStatus(window.__AC_LOCAL_ONLY
        ? 'Local-only mode - your appointment stays on this device until you return to normal mode.'
        : auth ? 'Working locally - checking Cloud in the background.' : 'Working locally - connect Cloud for cross-device sync.', '');
      return;
    }
    const bridge = window.AppointmentCompanionBridge;
