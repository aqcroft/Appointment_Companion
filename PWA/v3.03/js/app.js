...
function renderEnergy() {
  ...
  return `<section class="card service-workspace energy-workspace" id="energyPanel">
    ...
    <div class="energy-top-row"><label class="field region-field"><span>Region</span><select data-field="energy.region">${REGIONS.map(([id,label]) => `<option value="${id}"${selected(energy.region,id)}>${label}</option>`).join('')}</select></label><div class="compact-status"><small>UW tariff</small><strong>${escapeHtml(tariffLabel)}</strong><span>${result.rules.energyTariff || 1}-service rate</span></div></div>
    <div class="compare-grid energy-core-compare">
      <div class="compare-column current-column"><div class="compare-label">CURRENT</div>${currentBlock}<details class="advanced compact-advanced" ${energy.currentCostMode !== 'monthly' ? 'open' : ''}><summary>Different cost setup</summary><div class="segmented wrap"><button class="${on(energy.currentCostMode,'monthly')}" type="button" data-choice="energy.currentCostMode" data-value="monthly">One payment</button><button class="${on(energy.currentCostMode,'split')}" type="button" data-choice="energy.currentCostMode" data-value="split">Separate</button><button class="${on(energy.currentCostMode,'annual')}" type="button" data-choice="energy.currentCostMode" data-value="annual">Annual</button></div></details></div>
      <div class="compare-column uw-column"><div class="compare-label">UW / QUOTE USAGE</div>${energy.fuel !== 'gas' ? `<label class="field"><span>Electricity <small>National database</small></span>${field('energy.electricityUwKwh',energy.electricityUwKwh,'type="number" inputmode="numeric" min="0" placeholder="kWh/year"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Gas <small>National database</small></span>${field('energy.gasUwKwh',energy.gasUwKwh,'type="number" inputmode="numeric" min="0" placeholder="kWh/year"')}</label>` : ''}<details class="advanced compact-advanced"><summary>Different usage</summary><div class="usage-grid">${energy.fuel !== 'gas' ? usageSourceBlock('electricity','Electricity',[['Low',1600],['Typical',2500],['High',3800]]) : ''}${energy.fuel !== 'electricity' ? usageSourceBlock('gas','Gas',[['Low',7500],['Typical',11500],['High',17000]]) : ''}</div></details></div>
    </div>
    <div class="uw-price-strip"><span>UW Energy</span><strong>${result.uw.energy > 0 ? `£${money(result.uw.energy)}/m` : 'Waiting for usage'}</strong><small>${escapeHtml(tariffLabel)} · ${regionLabel}</small></div>

    ${energy.fuel !== 'gas' ? `<section class="workspace-section compact-options"><label class="compact-toggle full"><span>Peak / off-peak</span><span class="switch-control"><input type="checkbox" data-field="energy.peakOffPeak"${checked(energy.peakOffPeak)}><i></i></span></label>${energy.peakOffPeak ? `<div class="segmented field-gap"><button class="${on(energy.electricityProfile,'economy7')}" type="button" data-choice="energy.electricityProfile" data-value="economy7">Economy 7</button><button class="${on(energy.electricityProfile,'ev')}" type="button" data-choice="energy.electricityProfile" data-value="ev">EV</button></div><div class="grid-2 field-gap"><label class="field"><span>Day / peak kWh</span>${field('energy.dayKwh',energy.dayKwh,'type="number" min="0"')}</label><label class="field"><span>Night / off-peak kWh</span>${field('energy.nightKwh',energy.nightKwh,'type="number" min="0"')}</label></div><details class="advanced"><summary>Estimate day/night split</summary><div class="grid-2"><label class="field"><span>Bill day kWh</span>${field('energy.splitSampleDayKwh',energy.splitSampleDayKwh,'type="number" min="0"')}</label><label class="field"><span>Bill night kWh</span>${field('energy.splitSampleNightKwh',energy.splitSampleNightKwh,'type="number" min="0"')}</label></div><p class="notice">${split.annualDayKwh || split.annualNightKwh ? `${split.dayPercent.toFixed(1)}% day / ${split.nightPercent.toFixed(1)}% night → ${split.annualDayKwh} / ${split.annualNightKwh} kWh.` : 'Add matching day and night bill figures.'}</p><button class="secondary" type="button" data-apply-split ${split.annualDayKwh || split.annualNightKwh ? '' : 'disabled'}>Apply split</button></details><details class="advanced"><summary>Current peak/off-peak rates</summary><div class="grid-3"><label class="field"><span>Day p/kWh</span>${field('energy.currentDayRate',energy.currentDayRate,'type="number" min="0" step="0.01"')}</label><label class="field"><span>Night p/kWh</span>${field('energy.currentNightRate',energy.currentNightRate,'type="number" min="0" step="0.01"')}</label><label class="field"><span>Standing p/day</span>${field('energy.currentStandingCharge',energy.currentStandingCharge,'type="number" min="0" step="0.01"')}</label></div><label class="field"><span>Standard alternative annual cost</span>${field('energy.e7StandardAnnualCost',energy.e7StandardAnnualCost,'type="number" min="0" step="0.01"')}</label>${e7CurrentAnnual && energy.e7StandardAnnualCost ? `<p class="notice ${e7Saving < 0 ? 'warn' : ''}">${e7Saving >= 0 ? `Standard could save about £${money(e7Saving)}/year.` : `Economy 7 is about £${money(Math.abs(e7Saving))}/year lower.`}</p>` : ''}</details>` : ''}</section>` : ''}

    ${indicativePanel()}

    <section class="workspace-section compact-options"><label class="compact-toggle full"><span>Exit fees</span><span class="switch-control"><input type="checkbox" data-field="energy.exitFeesApply"${checked(energy.exitFeesApply)} aria-label="Energy exit fees apply"><i></i></span></label>${energy.exitFeesApply ? `<div class="grid-2 field-gap">${energy.fuel !== 'gas' ? `<label class="field"><span>Electricity exit fee</span>${field('energy.electricityExitFee',energy.electricityExitFee,'type="number" min="0"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Gas exit fee</span>${field('energy.gasExitFee',energy.gasExitFee,'type="number" min="0"')}</label>` : ''}</div>` : ''}</section>
...
function renderBroadband() {
  ...
  return `<section class="card service-workspace broadband-workspace" id="broadbandPanel">
    ...
    <div class="compare-grid">
      <div class="compare-column current-column"><div class="compare-label">CURRENT</div><label class="field"><span>Monthly cost</span>${field('broadband.currentMonthly',bb.currentMonthly,'type="number" min="0" step="0.01" placeholder="£ per month"')}</label><details class="advanced compact-advanced" ${bb.currentSpeed ? 'open' : ''}><summary>Current speed</summary><label class="field"><span>Speed / note</span>${field('broadband.currentSpeed',bb.currentSpeed,'placeholder="e.g. 100 Mbps"')}</label></details></div>
      <div class="compare-column uw-column"><div class="compare-label">UW</div>${chosen ? `<div class="selected-service-price"><strong>${escapeHtml(chosen.label)}</strong><span>£${money(result.uw.broadband)}/m</span></div>` : '<button class="choose-above-button" type="button" data-service-setup="broadband">Choose package ↑</button>'}</div>
    </div>
    <div class="compact-option-list">
      ...
      <label class="compact-toggle full"><span>Exit fees</span><span class="switch-control"><input type="checkbox" data-field="broadband.exitFeesApply"${checked(bb.exitFeesApply)}><i></i></span></label>${bb.exitFeesApply ? `<label class="field"><span>Broadband exit fee</span>${field('broadband.exitFee',bb.exitFee,'type="number" min="0"')}</label>` : ''}
    </div>
  </section>`;
}
