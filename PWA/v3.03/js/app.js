...
function renderEnergy() {
  ...
  return `<section class="card service-workspace energy-workspace" id="energyPanel">
    ...
    <div class="compact-config grid-2"><label class="field"><span>Region</span><select data-field="energy.region">${REGIONS.map(([id,label]) => `<option value="${id}"${selected(energy.region,id)}>${label}</option>`).join('')}</select></label><div class="field"><span>Fuel</span><div class="segmented"><button class="${on(energy.fuel,'electricity')}" type="button" data-choice="energy.fuel" data-value="electricity">Electricity</button><button class="${on(energy.fuel,'gas')}" type="button" data-choice="energy.fuel" data-value="gas">Gas</button><button class="${on(energy.fuel,'dual')}" type="button" data-choice="energy.fuel" data-value="dual">Dual</button></div></div></div>

    <section class="workspace-section"><div class="section-title"><div><h3>UW / quote usage</h3><p>National database figures from the UW quote are the normal starting point.</p></div></div>
      <div class="usage-grid">${energy.fuel !== 'gas' ? `<label class="field"><span>Electricity annual usage <small>National database</small></span>${field('energy.electricityUwKwh',energy.electricityUwKwh,'type="number" inputmode="numeric" min="0" placeholder="kWh/year"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Gas annual usage <small>National database</small></span>${field('energy.gasUwKwh',energy.gasUwKwh,'type="number" inputmode="numeric" min="0" placeholder="kWh/year"')}</label>` : ''}</div>
      <details class="advanced compact-advanced"><summary>Use different usage</summary><div class="usage-grid">${energy.fuel !== 'gas' ? usageSourceBlock('electricity','Electricity',[['Low',1600],['Typical',2500],['High',3800]]) : ''}${energy.fuel !== 'electricity' ? usageSourceBlock('gas','Gas',[['Low',7500],['Typical',11500],['High',17000]]) : ''}</div></details>
    </section>

    <section class="workspace-section"><div class="section-title"><div><h3>Monthly comparison</h3><p>Current on the left. UW on the right.</p></div></div>
      <div class="compare-grid">
        <div class="compare-column current-column"><div class="compare-label">CURRENT</div>
          ${energy.currentCostMode === 'monthly' ? `<label class="field"><span>Monthly payment</span>${field('energy.currentMonthly',energy.currentMonthly,'type="number" min="0" step="0.01" placeholder="£ per month"')}</label>` : energy.currentCostMode === 'split' ? `<div class="grid-2">${energy.fuel !== 'gas' ? `<label class="field"><span>Electricity / month</span>${field('energy.currentElectricityMonthly',energy.currentElectricityMonthly,'type="number" min="0" step="0.01"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Gas / month</span>${field('energy.currentGasMonthly',energy.currentGasMonthly,'type="number" min="0" step="0.01"')}</label>` : ''}</div>` : `<div class="grid-2">${energy.fuel !== 'gas' ? `<label class="field"><span>Annual electricity</span>${field('energy.annualElectricityCost',energy.annualElectricityCost,'type="number" min="0" step="0.01"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Annual gas</span>${field('energy.annualGasCost',energy.annualGasCost,'type="number" min="0" step="0.01"')}</label>` : ''}</div>`}
          <details class="advanced compact-advanced" ${energy.currentCostMode !== 'monthly' ? 'open' : ''}><summary>Different current-cost setup</summary><div class="segmented wrap"><button class="${on(energy.currentCostMode,'monthly')}" type="button" data-choice="energy.currentCostMode" data-value="monthly">One monthly payment</button><button class="${on(energy.currentCostMode,'split')}" type="button" data-choice="energy.currentCostMode" data-value="split">Separate payments</button><button class="${on(energy.currentCostMode,'annual')}" type="button" data-choice="energy.currentCostMode" data-value="annual">Annual bill</button></div></details>
        </div>
        <div class="compare-column uw-column"><div class="compare-label">UW</div><div class="auto-price"><small>${escapeHtml(tariffLabel)} · ${result.rules.energyTariff || 1}-service rate</small><strong>${result.uw.energy > 0 ? `£${money(result.uw.energy)}/month` : 'Calculated from usage'}</strong><span>Live tariff data</span></div></div>
      </div>
    </section>

    ${energy.fuel !== 'gas' ? `<section class="workspace-section"><label class="essential-row clickable service-option-row"><span class="essential-copy"><strong>Peak &amp; off-peak usage?</strong><small>Only switch on for Economy 7 or EV-style day/night usage.</small></span><span class="switch-control"><input type="checkbox" data-field="energy.peakOffPeak"${checked(energy.peakOffPeak)}><i></i></span></label>${energy.peakOffPeak ? `<div class="segmented field-gap"><button class="${on(energy.electricityProfile,'economy7')}" type="button" data-choice="energy.electricityProfile" data-value="economy7">Economy 7</button><button class="${on(energy.electricityProfile,'ev')}" type="button" data-choice="energy.electricityProfile" data-value="ev">EV</button></div><div class="grid-2 field-gap"><label class="field"><span>Annual day / peak usage</span>${field('energy.dayKwh',energy.dayKwh,'type="number" min="0"')}</label><label class="field"><span>Annual night / off-peak usage</span>${field('energy.nightKwh',energy.nightKwh,'type="number" min="0"')}</label></div><details class="advanced"><summary>Estimate annual day/night split</summary><div class="grid-2"><label class="field"><span>Bill-period day kWh</span>${field('energy.splitSampleDayKwh',energy.splitSampleDayKwh,'type="number" min="0"')}</label><label class="field"><span>Bill-period night kWh</span>${field('energy.splitSampleNightKwh',energy.splitSampleNightKwh,'type="number" min="0"')}</label></div><p class="notice">${split.annualDayKwh || split.annualNightKwh ? `${split.dayPercent.toFixed(1)}% day / ${split.nightPercent.toFixed(1)}% night → ${split.annualDayKwh} day and ${split.annualNightKwh} night kWh/year.` : 'Add matching day and night figures from a recent bill period.'}</p><button class="secondary" type="button" data-apply-split ${split.annualDayKwh || split.annualNightKwh ? '' : 'disabled'}>Apply split</button></details><details class="advanced"><summary>Current peak/off-peak rates</summary><div class="grid-3"><label class="field"><span>Day p/kWh</span>${field('energy.currentDayRate',energy.currentDayRate,'type="number" min="0" step="0.01"')}</label><label class="field"><span>Night p/kWh</span>${field('energy.currentNightRate',energy.currentNightRate,'type="number" min="0" step="0.01"')}</label><label class="field"><span>Standing p/day</span>${field('energy.currentStandingCharge',energy.currentStandingCharge,'type="number" min="0" step="0.01"')}</label></div><label class="field"><span>Indicative standard alternative annual cost</span>${field('energy.e7StandardAnnualCost',energy.e7StandardAnnualCost,'type="number" min="0" step="0.01"')}</label>${e7CurrentAnnual && energy.e7StandardAnnualCost ? `<p class="notice ${e7Saving < 0 ? 'warn' : ''}">${e7Saving >= 0 ? `Moving off Economy 7 could save about £${money(e7Saving / 12)}/month · £${money(e7Saving)}/year.` : `Economy 7 is about £${money(Math.abs(e7Saving))}/year lower.`}</p>` : ''}</details>` : ''}</section>` : ''}

    ${indicativePanel()}

    <section class="workspace-section exit-fee-section"><label class="essential-row clickable service-option-row"><span class="essential-copy"><strong>Exit fees apply?</strong><small>Leave off unless the current supplier charges one.</small></span><span class="switch-control"><input type="checkbox" data-field="energy.exitFeesApply"${checked(energy.exitFeesApply)} aria-label="Energy exit fees apply"><i></i></span></label>${energy.exitFeesApply ? `<div class="grid-2 field-gap">${energy.fuel !== 'gas' ? `<label class="field"><span>Electricity exit fee</span>${field('energy.electricityExitFee',energy.electricityExitFee,'type="number" min="0"')}</label>` : ''}${energy.fuel !== 'electricity' ? `<label class="field"><span>Gas exit fee</span>${field('energy.gasExitFee',energy.gasExitFee,'type="number" min="0"')}</label>` : ''}</div>` : ''}</section>

    <details class="advanced"><summary>Confirmed quote &amp; Energy adjustments</summary><div class="segmented wrap"><button class="${on(energy.uwQuoteMode,'single')}" type="button" data-choice="energy.uwQuoteMode" data-value="single">Manual single quote</button><button class="${on(energy.uwQuoteMode,'tiers')}" type="button" data-choice="energy.uwQuoteMode" data-value="tiers">Live / bundle tiers</button></div><div class="grid-3 field-gap">${energy.uwQuoteMode === 'single' ? `<label class="field"><span>Confirmed UW monthly amount</span>${field('energy.uwMonthly',energy.uwMonthly,'type="number" min="0" step="0.01"')}</label>` : [1,2,3].map(tier => `<label class="field"><span>${tier}-service / month</span>${field(`energy.uwTier${tier}`,energy[`uwTier${tier}`],'type="number" min="0" step="0.01"')}</label>`).join('')}</div><label class="toggle-row"><input type="checkbox" data-field="energy.adjustmentEnabled"${checked(energy.adjustmentEnabled)}><span><strong>Use a manual Energy adjustment</strong><small>Only for an awkward case the normal journey cannot represent.</small></span></label>${energy.adjustmentEnabled ? `<div class="grid-3"><label class="field"><span>Apply to</span><select data-field="energy.adjustmentTarget"><option value="current"${selected(energy.adjustmentTarget,'current')}>Current</option><option value="uw"${selected(energy.adjustmentTarget,'uw')}>UW</option></select></label><label class="field"><span>Period</span><select data-field="energy.adjustmentPeriod"><option value="monthly"${selected(energy.adjustmentPeriod,'monthly')}>Monthly</option><option value="annual"${selected(energy.adjustmentPeriod,'annual')}>Annual</option></select></label><label class="field"><span>Amount</span>${field('energy.adjustmentAmount',energy.adjustmentAmount,'type="number" min="0" step="0.01"')}</label></div>` : ''}</details>
  </section>`;
}
...
function renderBroadband() {
  ...
  const uwTotal = chosen ? Math.ceil(Number(chosen.monthly)) + (bb.wholeHomeWifi ? UW_RULES_2026_10_01.broadband.wholeHomeWifiMonthly : 0) + Number(bb.homePhoneMonthly || 0) : 0;
  return `<section class="card service-workspace broadband-workspace" id="broadbandPanel"><div class="workspace-heading"><span class="feature-icon broadband">🛜</span><div><div class="eyebrow">Broadband</div><h2>Broadband</h2><p>Add the current total, then choose the closest UW connection and speed.</p></div></div>
    ...
    <details class="advanced compact-advanced" ${bb.homePhoneMonthly ? 'open' : ''}><summary>UW phone add-on</summary><label class="field"><span>Home phone / month</span>${field('broadband.homePhoneMonthly',bb.homePhoneMonthly,'type="number" min="0" step="0.01"')}</label></details>
    <label class="essential-row clickable service-option-row"><span class="essential-copy"><strong>Exit fees apply?</strong><small>Leave off unless the current provider charges one.</small></span><span class="switch-control"><input type="checkbox" data-field="broadband.exitFeesApply"${checked(bb.exitFeesApply)}><i></i></span></label>${bb.exitFeesApply ? `<label class="field field-gap"><span>Broadband exit fee</span>${field('broadband.exitFee',bb.exitFee,'type="number" min="0"')}</label>` : ''}
  </section>`;
}

function renderMobile() {
  const mobile = appointment.mobile;
  return `<section class="card service-workspace mobile-workspace" id="mobilePanel"><div class="workspace-heading"><span class="feature-icon mobile">📱</span><div><div class="eyebrow">Mobile</div><h2>Mobile</h2><p>Choose the number of SIMs, add current costs and select the UW plans.</p></div></div>
    <div class="mobile-toolbar"><div class="segmented">${[1,2,3,4,5].map(count => `<button class="${on(mobile.simCount,count)}" type="button" data-sim-count="${count}">${count}</button>`).join('')}</div>${mobile.simCount > 1 ? `<button class="text-button" type="button" data-toggle-sim-names>${mobile.showNames ? 'Hide SIM names' : 'Name SIMs'}</button>` : ''}</div>
    <div class="stack field-gap">${mobile.sims.map((sim,index) => `<div class="sim compact-sim"><div class="sim-title"><strong>${escapeHtml(mobile.showNames ? sim.name || `SIM ${index+1}` : `SIM ${index+1}`)}</strong>${mobile.showNames ? `<label class="field inline-name"><span>Name</span>${field(`mobile.sims.${index}.name`,sim.name,'maxlength="40" placeholder="Optional name"')}</label>` : ''}</div><div class="compare-grid"><div class="compare-column current-column"><div class="compare-label">CURRENT</div><label class="field"><span>Monthly cost</span>${field(`mobile.sims.${index}.currentMonthly`,sim.currentMonthly,'type="number" min="0" step="0.01"')}</label></div><div class="compare-column uw-column"><div class="compare-label">UW</div><div class="plan-grid compact-plans"><button class="plan-option${on(sim.planId,'essentialMax')}" type="button" data-sim-plan="${index}" data-value="essentialMax"><strong>Go Essentials</strong><span>£6/m</span></button><button class="plan-option${on(sim.planId,'unlimitedMax')}" type="button" data-sim-plan="${index}" data-value="unlimitedMax"><strong>Go Unlimited</strong><span>£13/m</span></button></div><div class="auto-price compact"><small>Selected plan</small><strong>${sim.uwMonthly ? `£${money(sim.uwMonthly)}/month` : 'Choose a plan'}</strong></div></div></div><label class="essential-row clickable service-option-row"><span class="essential-copy"><strong>Exit fees apply?</strong><small>Only switch on for this SIM if needed.</small></span><span class="switch-control"><input type="checkbox" data-field="mobile.sims.${index}.exitFeesApply"${checked(sim.exitFeesApply)}><i></i></span></label>${sim.exitFeesApply ? `<label class="field field-gap"><span>Exit fee</span>${field(`mobile.sims.${index}.exitFee`,sim.exitFee,'type="number" min="0"')}</label>` : ''}${sim.planId === 'unlimitedMax' && mobile.sims.slice(0,index).some(item => item.include && item.planId === 'unlimitedMax') ? '<p class="notice">Additional Go Unlimited: first 3 months free, then £13/month.</p>' : ''}</div>`).join('')}</div>
  </section>`;
}

function renderBoilerCover() {
  const boiler = appointment.boilerCover;
  return `<section class="card service-workspace boiler-workspace" id="boilerPanel"><div class="workspace-heading"><span class="feature-icon boiler">🛠️</span><div><div class="eyebrow">Boiler Cover</div><h2>Boiler Cover</h2><p>One simple Current vs UW comparison.</p></div></div><div class="compare-grid"><div class="compare-column current-column"><div class="compare-label">CURRENT</div><label class="field"><span>Monthly cost</span>${field('boilerCover.currentMonthly',boiler.currentMonthly,'type="number" min="0" step="0.01"')}</label></div><div class="compare-column uw-column"><div class="compare-label">UW</div><div class="auto-price"><small>Boiler Cover</small><strong>£${money(boiler.monthly)}/month</strong></div></div></div><label class="essential-row clickable service-option-row"><span class="essential-copy"><strong>Exit fees apply?</strong><small>Leave off unless existing cover has a cancellation fee.</small></span><span class="switch-control"><input type="checkbox" data-field="boilerCover.exitFeesApply"${checked(boiler.exitFeesApply)}><i></i></span></label>${boiler.exitFeesApply ? `<label class="field field-gap"><span>Exit fee</span>${field('boilerCover.exitFee',boiler.exitFee,'type="number" min="0" step="0.01"')}</label>` : ''}</section>`;
}

function renderAdjustments() {
  const a = appointment.adjustments;
  const cashbackResult = calculateAppointment(appointment).cashback;
  return `<section class="card"><details class="advanced"><summary>Cashback Card &amp; advanced adjustments</summary><label class="person-row"><input type="checkbox" data-field="cashback.enabled"${checked(appointment.cashback.enabled)}><span><strong>Include Cashback Card estimate</strong><small>Average estimate is used by default.</small></span></label>${appointment.cashback.enabled ? `<div class="cashback-slider"><div class="slider-heading"><span><strong>Monthly card spend</strong><small>Adjust to roughly match the household.</small></span><output data-cashback-output>£${money(appointment.cashback.monthlySpend)}/m</output></div><input type="range" min="0" max="2500" step="50" value="${Number(appointment.cashback.monthlySpend || 0)}" data-field="cashback.monthlySpend" aria-label="Monthly Cashback Card spend"><p class="hint">Current estimate: about £${money(cashbackResult.selected)}/month cashback before the card fee.</p><details class="advanced compact-advanced"><summary>Change estimate basis</summary><div class="pills"><button class="pill${on(appointment.cashback.tier,'low')}" type="button" data-choice="cashback.tier" data-value="low">Low</button><button class="pill${on(appointment.cashback.tier,'average')}" type="button" data-choice="cashback.tier" data-value="average">Average</button><button class="pill${on(appointment.cashback.tier,'high')}" type="button" data-choice="cashback.tier" data-value="high">High</button></div></details></div>` : ''}<label class="person-row"><input type="checkbox" data-field="adjustments.recurringEnabled"${checked(a.recurringEnabled)}><span><strong>Manual recurring adjustment</strong></span></label>${a.recurringEnabled ? `<div class="grid-2"><label class="field"><span>Current amount</span>${field('adjustments.currentAmount',a.currentAmount,'type="number" min="0" step="0.01"')}</label><label class="field"><span>UW amount</span>${field('adjustments.uwAmount',a.uwAmount,'type="number" min="0" step="0.01"')}</label><label class="field"><span>Current reason</span>${field('adjustments.currentReason',a.currentReason)}</label><label class="field"><span>UW reason</span>${field('adjustments.uwReason',a.uwReason)}</label><label class="field"><span>Period</span><select data-field="adjustments.period"><option value="monthly"${selected(a.period,'monthly')}>Monthly</option><option value="annual"${selected(a.period,'annual')}>Annual</option></select></label></div>` : ''}<label class="person-row"><input type="checkbox" data-field="adjustments.oneOffEnabled"${checked(a.oneOffEnabled)}><span><strong>One-off benefit or charge</strong></span></label>${a.oneOffEnabled ? `<div class="grid-3"><label class="field"><span>Type</span><select data-field="adjustments.oneOffType"><option value="benefit"${selected(a.oneOffType,'benefit')}>Benefit/refund</option><option value="charge"${selected(a.oneOffType,'charge')}>Charge</option></select></label><label class="field"><span>Amount</span>${field('adjustments.oneOffAmount',a.oneOffAmount,'type="number" min="0"')}</label><label class="field"><span>Label</span>${field('adjustments.oneOffLabel',a.oneOffLabel)}</label></div>` : ''}</details></section>`;
}
